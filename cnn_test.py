...
class Model(nn.Module):
    ...
    def __init__(self, input_size=784, hidden_dim=200, output_dim=1, two_layer=False):
        ...
        self.single_layer = nn.Sequential(
                            nn.Linear(input_size, output_dim, bias=True),
                            )
...
def main(args):
    ...
    while optimizer.grad_update < args.max_iter:
        ...
        loss, accuracy = test(model, test_dataloader)
        losses_gn_half_sketch_val.append(loss)
        print('Test Loss: {}, Accuracy: {}'.format(loss, accuracy))
    time_end = time.time()
    ...
    while optimizer.grad_update < args.max_iter:
        ...
        loss, accuracy = test(model, test_dataloader)
        losses_gn_sketch_val.append(loss)
        print('Test Loss: {}, Accuracy: {}'.format(loss, accuracy))
    time_end = time.time()
    ...
    plt.semilogy(np.arange(len(losses_gn_sketch_val)) * t_solve_gn_sketch / len(losses_gn_sketch_val), losses_gn_sketch_val, 'g', 
                 np.arange(len(losses_gn_half_sketch_val)) * t_solve_gn_half_sketch_val / len(losses_gn_half_sketch_val), losses_gn_half_sketch_val, 'r', 
                 np.arange(len(losses_sgd_val)) * t_solve_sgd / len(losses_sgd_val),losses_sgd_val, 'b',
                 np.arange(len(losses_adam_val)) * t_solve_adam_val / len(losses_adam_val), losses_adam_val, 'c')
