...
class VGG(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            ...
            # TODO: convolutional layer, input channels 3, output channels 8, filter size 3
            # TODO: max-pooling layer, size 2
            nn.Conv2d(1, 8, kernel_size=3, padding=1),#, stride=1,padding=1,bias=True),
            nn.MaxPool2d(2),
...
def main(args):
    ...
    while optimizer.grad_update < args.max_iter:
        ...
        optimizer, losses_gn, vlosses_gn)
    time_end = time.time()
    ...
    plt.subplot(2, 1, 2)
    plt.plot(np.arange(len(vlosses_gn)) * t_solve_gn / len(vlosses_gn), vlosses_gn, 'k',
                 np.arange(len(vlosses_gn_sketch)) * t_solve_gn_sketch /len(vlosses_gn_sketch), vlosses_gn_sketch, 'g', 
...
def train_GN(model, dataloader, valLoader, optimizer, all_losses, val_losses):
   ...
   for idx, data in enumerate(dataloader):
       ...
       def closure():
           ...
           loss = F.cross_entropy(pred,labels.type(torch.LongTensor).squeeze())
