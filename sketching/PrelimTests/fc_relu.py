...
class fc_relu_Classifier(object):
  """
  ...
  def loss(self, X, y=None):
    ...
    if not self.twoLayer:
        ...
        scores = np.round(result).astype(int)
    else:
        ...
        scores = np.round(result_fc2).astype(int)
    ...
    ############################################################################
    if not self.twoLayer:
        loss, dscores = mse_loss(result, y)
        _, dW1, db1 = fc_backward(dscores.reshape(dscores.size,1), cache_fc)
        grads['b1'] = db1
        grads['W1'] = dW1
    else:
        loss, dscores = mse_loss(result_fc2, y)
